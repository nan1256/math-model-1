...
def generate_markdown_summary(builder, predictor, df_engineered,
    ...
    with open(summary_file, 'w', encoding='utf-8') as f:
        ...
        f.write("$$Y = \\alpha_s \\cdot t + \\beta_{\\log} \\ln(1+t) + \\varepsilon$$\n\n")
        f.write("**用途**：机理分析——验证时间延迟是否主导、比较四类样品基础时间衰减斜率、")
        f.write("通过锈蚀钢筋斜率 > 普通钢筋斜率说明锈蚀状态降低剩磁稳定性。")
        f.write("仅包含 type:day 时间斜率和 log_t 项，不含 Rust0*t（后者与 type:day 共线），")
        f.write("也不含环境累计变量，避免解释困难。\n\n")
        ...
        f.write("Leave-One-Sample-Out CV 用于评估模型对同分布新样品的泛化能力。")
        f.write("结果表明，在当前实验样品类型和环境范围内，")
        f.write("Model 5 对未参与训练的样品仍保持较低预测误差，")
        f.write("说明模型具有较好的样品层面泛化能力。\n\n")
        ...
        f.write("该区间基于 Model 5 在 Y 空间的残差标准差构造，属于近似预测区间，")
        f.write("用于实际判定时的不确定性提示，而非严格贝叶斯置信区间。\n\n")
        ...
        f.write("9. **Leave-One-Sample-Out CV 表明，在同类样品与相同实验环境条件下**，")
        f.write("Model 5 对未参与训练的样品仍保持较低预测误差，")
        f.write("说明模型具有较好的样品层面泛化能力。\n\n")
        f.write("10. **动态阈值区间比单点阈值更适合实际鉴定**：")
        f.write("基于残差标准差的近似预测区间可提供置信度分级，")
        f.write("避免单点阈值在高不确定性场景下的误判。\n\n")
        f.write("11. **解释模型用于机理分析，预测模型用于动态阈值和工程应用**。")
        f.write("解释模型仅包含样品类型时间衰减斜率和 log_t 项，便于机理比较；")
